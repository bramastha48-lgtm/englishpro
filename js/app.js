...
// ---- DICTIONARY ----
var DICT_STATE = { history: JSON.parse(localStorage.getItem('ep_dict_history') || '[]'), saved: JSON.parse(localStorage.getItem('ep_dict_saved') || '[]'), currentWord: null, debounceTimer: null };
function saveDictHistory(){ localStorage.setItem('ep_dict_history', JSON.stringify(DICT_STATE.history)); }
function saveDictSaved(){ localStorage.setItem('ep_dict_saved', JSON.stringify(DICT_STATE.saved)); }
function renderDictionary(){ if('speechSynthesis' in window) window.speechSynthesis.cancel(); $('#dict-loading').classList.add('hidden'); $('#dict-error').classList.add('hidden'); $('#dict-result').classList.add('hidden'); $('#dict-suggestions').classList.add('hidden'); renderDictHistory(); setTimeout(function(){ var i=$('#dict-input'); if(i) i.focus(); },100); }
function dictSearch(word){ if(!word||!word.trim()) return; word=word.trim().toLowerCase(); $('#dict-loading').classList.remove('hidden'); $('#dict-error').classList.add('hidden'); $('#dict-result').classList.add('hidden'); $('#dict-suggestions').classList.add('hidden'); $('#dict-history').classList.add('hidden'); fetch('https://api.dictionaryapi.dev/api/v2/entries/en/'+encodeURIComponent(word)).then(function(r){ if(!r.ok) throw new Error('x'); return r.json(); }).then(function(data){ $('#dict-loading').classList.add('hidden'); renderDictResult(data[0]); addToDictHistory(word); }).catch(function(){ $('#dict-loading').classList.add('hidden'); showDictError(word); }); }
function showDictError(word){ $('#dict-error').classList.remove('hidden'); $('#dict-result').classList.add('hidden'); $('#dict-error-title').textContent=S.lang==='id'? 'Kata "'+word+'" tidak ditemukan' : 'Word "'+word+'" not found'; $('#dict-error-text').textContent=S.lang==='id'? 'Coba cari kata lain atau periksa ejaannya.' : 'Try searching for another word or check the spelling.'; }
function renderDictResult(entry){ DICT_STATE.currentWord=entry.word; $('#dict-result').classList.remove('hidden'); $('#dict-history').classList.remove('hidden'); $('#dict-word').textContent=entry.word; var phonetic=''; if(entry.phonetics){ for(var i=0;i<entry.phonetics.length;i++){ if(entry.phonetics[i].text){ phonetic=entry.phonetics[i].text; break; } } } $('#dict-phonetic').textContent=phonetic; var audioUrl=''; if(entry.phonetics){ for(var i=0;i<entry.phonetics.length;i++){ if(entry.phonetics[i].audio){ audioUrl=entry.phonetics[i].audio; break; } } } var btnSpeak=$('#btn-dict-speak'); if(audioUrl){ btnSpeak.onclick=function(){ new Audio(audioUrl).play().catch(function(){ speak(entry.word); }); }; } else { btnSpeak.onclick=function(){ speak(entry.word); }; } var btnSave=$('#btn-dict-save'); var isSaved=DICT_STATE.saved.indexOf(entry.word)>=0; btnSave.textContent=isSaved?'⭐':'☆'; btnSave.classList.toggle('saved',isSaved); btnSave.onclick=function(){ var idx=DICT_STATE.saved.indexOf(entry.word); if(idx>=0){ DICT_STATE.saved.splice(idx,1); toast(S.lang==='id'?'Dihapus':'Removed'); } else { DICT_STATE.saved.push(entry.word); toast(S.lang==='id'?'💾 Kata disimpan!':'💾 Word saved!'); } saveDictSaved(); S.progress.wordsLearned[entry.word]=true; save(); var ns=DICT_STATE.saved.indexOf(entry.word)>=0; btnSave.textContent=ns?'⭐':'☆'; btnSave.classList.toggle('saved',ns); }; var mc=$('#dict-meanings'); mc.innerHTML=''; (entry.meanings||[]).forEach(function(meaning){ var card=document.createElement('div'); card.className='dict-meaning-card'; var defsHtml=''; (meaning.definitions||[]).slice(0,5).forEach(function(def,i){ defsHtml+='<div class="dict-definition"><div class="dict-def-text"><span class="dict-def-number">'+(i+1)+'.</span> '+escapeHtml(def.definition)+'</div>'+(def.example?'<div class="dict-def-example">"'+escapeHtml(def.example)+'"</div>':'')+'</div>'; }); card.innerHTML='<span class="dict-pos">'+escapeHtml(meaning.partOfSpeech)+'</span>'+defsHtml; mc.appendChild(card); }); var allSyn=[]; (entry.meanings||[]).forEach(function(m){ (m.synonyms||[]).forEach(function(s){ if(allSyn.indexOf(s)<0) allSyn.push(s); }); }); var synS=$('#dict-synonyms'), synL=$('#dict-syn-list'); if(allSyn.length>0){ synS.classList.remove('hidden'); $('#dict-syn-label').textContent=S.lang==='id'?'Sinonim':'Synonyms'; synL.innerHTML=allSyn.slice(0,10).map(function(s){ return '<button class="dict-tag" data-word="'+escapeHtml(s)+'">'+escapeHtml(s)+'</button>'; }).join(''); synL.querySelectorAll('.dict-tag').forEach(function(btn){ btn.addEventListener('click',function(){ $('#dict-input').value=btn.dataset.word; dictSearch(btn.dataset.word); }); }); } else { synS.classList.add('hidden'); } var allAnt=[]; (entry.meanings||[]).forEach(function(m){ (m.antonyms||[]).forEach(function(a){ if(allAnt.indexOf(a)<0) allAnt.push(a); }); }); var antS=$('#dict-antonyms'), antL=$('#dict-ant-list'); if(allAnt.length>0){ antS.classList.remove('hidden'); $('#dict-ant-label').textContent=S.lang==='id'?'Antonim':'Antonyms'; antL.innerHTML=allAnt.slice(0,10).map(function(a){ return '<button class="dict-tag" data-word="'+escapeHtml(a)+'">'+escapeHtml(a)+'</button>'; }).join(''); antL.querySelectorAll('.dict-tag').forEach(function(btn){ btn.addEventListener('click',function(){ $('#dict-input').value=btn.dataset.word; dictSearch(btn.dataset.word); }); }); } else { antS.classList.add('hidden'); } var srcUrl=entry.sourceUrls&&entry.sourceUrls[0]; var srcEl=$('#dict-source'); if(srcUrl){ srcEl.classList.remove('hidden'); $('#dict-source-url').innerHTML='Source: <a href="'+escapeHtml(srcUrl)+'" target="_blank" style="color:var(--primary-light)">'+escapeHtml(srcUrl)+'</a>'; } else { srcEl.classList.add('hidden'); } }
function addToDictHistory(word){ DICT_STATE.history=DICT_STATE.history.filter(function(w){return w!==word;}); DICT_STATE.history.unshift(word); if(DICT_STATE.history.length>20) DICT_STATE.history.pop(); saveDictHistory(); renderDictHistory(); }
function renderDictHistory(){ var c=$('#dict-history-list'), tt=$('#dict-history-title'); if(!c) return; tt.textContent=S.lang==='id'?'Pencarian Terakhir':'Recent Searches'; if(DICT_STATE.history.length===0){ c.innerHTML='<p style="color:var(--text2);font-size:13px">'+(S.lang==='id'?'Belum ada pencarian':'No recent searches')+'</p>'; return; } var html=DICT_STATE.history.map(function(w){ return '<button class="dict-history-item" data-word="'+escapeHtml(w)+'">🔍 '+escapeHtml(w)+'</button>'; }).join(''); html+='<button class="dict-history-clear" id="btn-dict-clear">🗑️ '+(S.lang==='id'?'Hapus':'Clear')+'</button>'; c.innerHTML=html; c.querySelectorAll('.dict-history-item').forEach(function(btn){ btn.addEventListener('click',function(){ $('#dict-input').value=btn.dataset.word; dictSearch(btn.dataset.word); }); }); var cb=$('#btn-dict-clear'); if(cb) cb.addEventListener('click',function(){ DICT_STATE.history=[]; saveDictHistory(); renderDictHistory(); }); }
function initDictionary(){ var input=$('#dict-input'), sb=$('#btn-dict-search'); if(!input||!sb) return; input.addEventListener('keydown',function(e){ if(e.key==='Enter'){ $('#dict-suggestions').classList.add('hidden'); dictSearch(input.value); } }); sb.addEventListener('click',function(){ $('#dict-suggestions').classList.add('hidden'); dictSearch(input.value); }); }
initDictionary();
})();
